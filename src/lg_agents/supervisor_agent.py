...
class SupervisorAgent(BaseGraphAgent):
    ...
    async def _route_request(
        ...
        try:
            # 마지막 메시지 추출
            filtered_messages = filter_messages(state["messages"], include_types=[HumanMessage])
...
패턴명만 영어 대문자로 응답하세요."""
            ...
            decision = response.content.strip().upper()  # 대문자로 변환
            ...
            state["messages"] = [
                AIMessage(
                    content=f"워크플로우 패턴 결정: {state['workflow_pattern']}"
                )
            ]
    ...
    def _get_next_step(self, state: SupervisorState):
        ...
        if not state["workflow_pattern"]:
            return Send(END)
        return Send(self.get_node_name("DATA_COLLECTOR"), state)

    def _get_next_after_data(self, state: SupervisorState):
        """DataCollector 후 다음 단계 결정"""
        if state["workflow_pattern"] is None:
            return Send(END)
        return Send(self.get_node_name("ANALYSIS"), state)

    def _get_next_after_analysis(self, state: SupervisorState):
        """Analysis 후 다음 단계 결정"""
        if state["workflow_pattern"] is None:
            return Send(END)
        return Send(self.get_node_name("TRADING"), state)
