#!/usr/bin/env python3
"""
Step 4.1: 통합 한국 주식시장 분석 플랫폼 (간소화 버전)
=========================================================

Redis 없이 실행 가능한 버전으로, 메모리 기반 상태 관리를 사용합니다.
모든 핵심 개념은 동일하게 유지하면서 설치와 실행을 단순화했습니다.
...
class InMemoryStateManager:
    """메모리 기반 상태 관리 (Redis 대체)"""
    
    def __init__(self):
        self.stock_data: Dict[str, StockData] = {}
        self.analyses: Dict[str, AnalysisResult] = {}
        self.subscribers: Dict[str, List[callable]] = {"stock_updates": [], "analysis_updates": []}
        
    async def initialize(self):
        """상태 관리자 초기화"""
        logger.info("메모리 기반 상태 관리자가 초기화되었습니다")
    ...
    async def update_stock_data(self, stock_data: StockData):
        ...
        # 구독자들에게 알림
        for callback in self.subscribers["stock_updates"]:
            try:
    ...
    async def update_analysis(self, analysis: AnalysisResult):
        """분석 결과 업데이트"""
        analysis_key = f"{analysis.stock_symbol}_{analysis.analysis_type.value}_{analysis.analyst_id}"
        self.analyses[analysis_key] = analysis
        
        # 구독자들에게 알림
        for callback in self.subscribers["analysis_updates"]:
            try:
...
class MarketDataFeed:
    ...
    def __init__(self, state_manager: InMemoryStateManager):
...
class TechnicalAnalysisWorkflow:
    ...
    async def analyze(self, symbol: str) -> AnalysisResult:
        """기술적 분석 실행"""
        config = {"configurable": {"thread_id": f"tech_analysis_{symbol}_{datetime.now().isoformat()}"}}
...
class AnalysisAgent:
    ...
    def __init__(self, agent_id: str, port: int, state_manager: InMemoryStateManager):
...
class WebSocketHandler:
    ...
    def __init__(self, state_manager: InMemoryStateManager):
        ...
        # 상태 변화 구독
        self.state_manager.subscribe("stock_updates", self.broadcast_stock_update)
        self.state_manager.subscribe("analysis_updates", self.broadcast_analysis_update)
    ...
    async def _send_initial_data(self, websocket):
        ...
        try:
            ...
            await websocket.send(json.dumps({
                'type': 'welcome',
                'message': '한국 주식시장 분석 플랫폼에 연결되었습니다',
                'stocks': ['005930:삼성전자', '373220:LG에너지솔루션', '000660:SK하이닉스']
            }))
...
async def main():
    ...
    try:
        ...
        print("\n" + "="*60)
        print("🚀 통합 한국 주식시장 분석 플랫폼 (간소화 버전)")
        print("="*60)
        print("📡 웹소켓 연결: ws://localhost:8765")
        print("🤖 기본적 분석 에이전트: http://localhost:8001")
        print("💭 센티먼트 분석 에이전트: http://localhost:8002") 
        print("⚠️  리스크 분석 에이전트: http://localhost:8003")
        print("\n📈 모니터링 대상:")
        print("   - 005930: 삼성전자")
        print("   - 373220: LG에너지솔루션")
        print("   - 000660: SK하이닉스")
        print("\n💡 사용법:")
        print("   1. client.html을 웹브라우저에서 열기")
        print("   2. '연결' 버튼 클릭하여 실시간 데이터 확인")
        print("   3. 분석 버튼들로 AI 분석 결과 확인")
        print("\n⏹️  Ctrl+C로 중지")
        print("="*60)
